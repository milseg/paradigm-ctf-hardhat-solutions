...
contract ScriptTrapdooor {
    /*Slighly modified for adapting to project structure*/
    bool solved = false;

    function run(uint expected, bytes memory code ) external {

        FactorizorLike factorizer = FactorizorLike(address(new Deployer(code)));
